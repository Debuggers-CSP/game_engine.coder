...
<div class="generator-card" id="world-map-builder">
  ...
  <style>
    .wm-wrap { max-width:1400px; margin:0 auto; padding:0 40px; display:flex; flex-direction:column; gap:12px; align-items:stretch; }
    ...
    .wm-bottom { display:grid; grid-template-columns: 1fr 1fr; gap:8px; align-items:stretch; }
    .wm-panel { background: rgba(0,0,0,0.35); border:1px solid rgba(255,215,0,0.25); border-radius:10px; padding:10px; display:flex; flex-direction:column; height:100%; }
    .wm-title { color:#ffd700; text-align:center; font-size:0.95em; margin-bottom:6px; }
    .wm-palette { display:flex; flex-direction:column; gap:8px; }
    .wm-group-title { color:#cfe3ff; font-size:0.9em; margin:2px 0; padding:3px 5px; border-left:3px solid rgba(255,215,0,0.35); background: rgba(255,215,0,0.06); border-radius:6px; }
    .wm-group-grid { display:grid; grid-template-columns: repeat(3, 1fr); gap:6px; }
    .wm-tile { display:flex; flex-direction:column; align-items:center; justify-content:center; gap:3px; border:1px solid rgba(255,215,0,0.25); border-radius:8px; padding:6px; cursor:pointer; background:rgba(255,215,0,0.08); }
    .wm-emoji { font-size:1.4em; filter:drop-shadow(0 0 4px rgba(255,215,0,0.25)); }
    .wm-controls { display:flex; flex-direction:column; gap:6px; justify-content:flex-start; align-items:stretch; }
    .wm-mini { padding:6px 10px; font-size:0.85em; width:100%; }
    @media (max-width: 1000px) { .wm-wrap { padding: 0 20px; } }
    @media (max-width: 700px) { .wm-bottom { grid-template-columns: 1fr; } }
  </style>
  <div class="wm-wrap">
    ...
    <div class="wm-bottom">
      ...
      <div class="wm-panel wm-tools-panel">
        <div class="wm-controls">
          ...
          <button id="wm-tool-erase" class="generator-btn wm-mini">🧽 Erase</button>
          <button id="wm-tool-bucket" class="generator-btn wm-mini">🪣 Bucket</button>
          <button id="wm-undo" class="generator-btn wm-mini">↩️ Undo</button>
          <button id="wm-clear" class="generator-btn wm-mini">🗑️ Clear</button>
          <button id="wm-export" class="generator-btn wm-mini">🖼️ Save PNG</button>
          <button id="wm-zoom-out" class="generator-btn wm-mini">➖ Zoom Out</button>
          <button id="wm-zoom-in" class="generator-btn wm-mini">➕ Zoom In</button>
          <button id="wm-zoom-reset" class="generator-btn wm-mini">🔄 Reset View</button>
        </div>
      </div>
    </div>
  </div>
  <script>
    (function(){
      ...
      const cols = 80, rows = 50, cell = 40; // big virtual grid
      ...
      function buildPalette(){
        ...
        GROUPS.forEach(group => {
          ...
          group.items.forEach(p => {
            ...
            d.addEventListener('click', () => { state.selected = { kind:'tile', category:p.category, subtype:p.subtype, color:p.color, emoji:p.emoji }; });
            grid.appendChild(d);
          });
        ...
        const pinEl = document.createElement('div'); pinEl.className='wm-tile';
        const pinEmoji = document.createElement('div'); pinEmoji.className='wm-emoji'; pinEmoji.textContent=PIN.emoji;
        const pinLabel = document.createElement('div'); pinLabel.style.color='#bfbfbf'; pinLabel.style.fontSize='0.85em'; pinLabel.textContent=PIN.label;
        pinEl.appendChild(pinEmoji); pinEl.appendChild(pinLabel);
        pinEl.addEventListener('click', () => { state.selected = { kind:'pin', emoji: PIN.emoji }; });
      ...
      document.getElementById('wm-tool-erase').addEventListener('click', ()=> state.tool='erase');
      document.getElementById('wm-tool-bucket').addEventListener('click', ()=> state.tool='bucket');
