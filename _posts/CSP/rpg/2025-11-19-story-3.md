...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = { persistent: null, typing: null, userEdited: false };
    ...
    function updateStepUI() {
        const current = steps[stepIndex];
        // Default: disable all, then enable per step
        [ui.bg, ui.pSprite, ui.pX, document.getElementById('movement-keys'), ui.nId, ui.nMsg, ui.nSprite, ui.nX, ui.nY]
            .forEach(el => { if (el) el.disabled = true; });
        if (current === 'background') {
            unlockField(ui.bg);
        } else if (current === 'player') {
            // Background locked after confirm; allow only player-related
            ...
            unlockField(ui.nId);
            unlockField(ui.nMsg);
            unlockField(ui.nSprite);
            unlockField(ui.nX);
            unlockField(ui.nY);
        } else if (current === 'freestyle') {
            // Free editing of code
            [ui.bg, ui.pSprite, ui.pX, document.getElementById('movement-keys'), ui.nId, ui.nMsg, ui.nSprite, ui.nX, ui.nY]
        .forEach(el => { 
            if (el) {
                el.disabled = false; 
...
export const gameLevelClasses = [CustomLevel];`;
    ...
    ui.editor.addEventListener('input', () => { state.userEdited = true; });
    ...
    function syncFromControlsIfFreestyle() {
        ...
        const newCode = generateStepCode('npc'); // full composition (bg + player + npcs)
        if (newCode) {
    ...
    document.getElementById('btn-confirm').addEventListener('click', () => {
        ...
        if (!newCode) {
            ...
            else alert('Fill NPC details or keep defaults, then Confirm Step.');
            return;
        }
        animateTypingDiff(oldCode, newCode, () => {
            // Lock fields for completed step and advance
            ...
            if (current === 'npc') { lockField(ui.nId); lockField(ui.nMsg); lockField(ui.nSprite); lockField(ui.nX); lockField(ui.nY); }
    ...
    // In freestyle mode, auto-sync code from controls unless user edited code
    const mvCtl = document.getElementById('movement-keys');
    [ui.bg, ui.pSprite, ui.pX, mvCtl].forEach(el => {
        if (el) {
            el.addEventListener('change', syncFromControlsIfFreestyle);
            el.addEventListener('input', syncFromControlsIfFreestyle);
        }
    });
    ui.npcs.forEach(slot => {
        [slot.nId, slot.nMsg, slot.nSprite, slot.nX, slot.nY].forEach(el => {
            if (el) {
                el.addEventListener('change', syncFromControlsIfFreestyle);
                el.addEventListener('input', syncFromControlsIfFreestyle);
            }
