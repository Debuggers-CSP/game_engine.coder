        ...
        <div class="glass-panel creator-panel">
            ...
            <div class="scroll-form">
                ...
                <div class="asset-group">
                    ...
                    <label>ID</label>
                    <input type="text" id="npc-id" value="" placeholder="NPC id">
                    <label>Message(show by clicking E)</label>
                    <input type="text" id="npc-msg" value="" placeholder="Message when interacted with">
                    <label>Sprite</label>
                    ...
                    <input type="range" id="npc-y" min="0" max="600" value="300">
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ui = {
        ...
        nId: document.getElementById('npc-id'),
        nMsg: document.getElementById('npc-msg'),
        nSprite: document.getElementById('npc-sprite'),
        nX: document.getElementById('npc-x'),
        nY: document.getElementById('npc-y'),
    ...
    const state = { persistent: null, typing: null };

        // Baseline skeleton: imports + empty CustomLevel with placeholders
...
class CustomLevel {
    constructor(gameEnv) {
        ...
        // Background configuration (set via Asset Configurations)
        // const bgData = { /* name, src, pixels */ };

        // Player configuration (set via Asset Configurations)
        // const playerData = { /* id, src, INIT_POSITION, pixels, orientation, etc. */ };

        // Optional NPC configuration (set via Asset Configurations)
        // const npcData = { /* id, greeting, src, INIT_POSITION, pixels, orientation, dialogues */ };

        // Define objects for this level (populate on Confirm Changes)
        this.classes = [
...
export const gameLevelClasses = [CustomLevel];`;
    ...
    function generateCode(includeNpc = false) {
        ...
        // Require minimal selections for code generation
        if (!ui.bg.value || !ui.pSprite.value) {
            return null; // block generation until user selects required fields
        }

        // Prepare optional NPC inputs
        const nId = (ui.nId && ui.nId.value ? ui.nId.value.trim() : 'NPC').replace(/'/g, "\\'");
        const nMsg = (ui.nMsg && ui.nMsg.value ? ui.nMsg.value.trim() : '').replace(/'/g, "\\'");
        const nSpriteKey = (ui.nSprite && ui.nSprite.value) ? ui.nSprite.value : 'chillguy';
        const nSprite = assets.sprites[nSpriteKey] || assets.sprites['chillguy'];
        const nX = (ui.nX && ui.nX.value) ? parseInt(ui.nX.value, 10) : 500;
        const nY = (ui.nY && ui.nY.value) ? parseInt(ui.nY.value, 10) : 300;

        const npcImport = includeNpc ? "\n    import Npc from '/assets/js/adventureGame/GameEngine/Npc.js';" : '';
        const npcBlock = includeNpc ? `
        // NPC configuration (static character with dialogue)
        const npcData = {
            id: '${nId}',
            greeting: '${nMsg}',
        ...
        const npcClassEntry = includeNpc ? ",\n      { class: Npc, data: npcData }" : '';
  ...
  constructor(gameEnv) {
    ...
    const bgData = {
      ...
      pixels: { height: ${bg.h}, width: ${bg.w} }
        ...
        this.classes = [
            { class: GameEnvBackground, data: bgData },
            { class: Player, data: playerData }${npcClassEntry}
        ];
  }
}
...
export const gameLevelClasses = [CustomLevel];`;
    ...
    document.getElementById('btn-confirm').addEventListener('click', () => {
        ...
        const newCode = generateCode(true); // include NPC on confirm
        if (!newCode) {
            alert('Please select a Background and Player Sprite before confirming.');
            return;
        }
    ...
    // Initial: show baseline imports and empty level; don't run until Confirm
