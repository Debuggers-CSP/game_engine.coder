...
// -------- Session helpers --------
const API_URL = (location.hostname === 'localhost') ? 'http://localhost:8304/api' : `${pythonURI}/api`;
...
async function loadStoryVotes() {
    try {
        ...
        // Fetch elements
        const origin = (location.hostname==='localhost'? 'http://localhost:8304' : window.location.origin);
