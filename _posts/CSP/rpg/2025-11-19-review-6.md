...
<div class="review-wrap">
    ...
    <div id="review-grid" class="review-grid">
        <div class="card" id="card-mode">
            ...
            <div class="actions">
                <a href="/rpg/content" class="link-btn">Edit Mode</a>
            </div>
        </div>
        ...
        <div class="card" id="card-character">
            ...
            <div class="actions">
                <a href="/rpg/game-creater" class="link-btn">Edit Character</a>
            </div>
        </div>
        ...
        <div class="card" id="card-visual">
            ...
            <div class="actions">
                <a href="/rpg/game-creater" class="link-btn">Adjust Appearance</a>
            </div>
        </div>
...
// -------- Session helpers --------
const API_URL = (location.hostname === 'localhost') ? 'http://localhost:8587/api' : `${pythonURI}/api`;
...
async function loadKeybindings(modeGuess) {
    ...
    try {
        ...
        // Fallback: if no bindings for this mode, try latest any mode
        if (res.status === 404) {
...
function drawAppearance(appearance) {
    ...
    // clear
    ctx.clearRect(0,0,canvas.width,canvas.height);
    // colors from appearance with fallbacks; support multiple creation schemas
    const normalizeColor = (c)=>{
        if (!c) return null;
        // Accept hex, rgb(a), named — pass through
        return String(c).trim();
    };
    const classId = appearance?.classId || appearance?.class || '';
    const hair = normalizeColor(appearance?.hairColor) || '#3B2F2F';
    // Cloth color: explicit clothingColor, else class-based default
