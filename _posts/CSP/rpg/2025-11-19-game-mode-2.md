---
layout: post
title: Game Mode Selection
description: Let the user choose the type of RPG they want to create, affecting the examples and style shown later.
permalink: /rpg/mode
comments: True
---

<script src="https://cdn.tailwindcss.com"></script>
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>

<div class="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 py-12 px-4 sm:px-6 lg:px-8">
  <div class="max-w-6xl mx-auto">
    <!-- Header Section -->
    <div class="text-center mb-16">
      <h1 class="text-4xl font-bold mb-4">Choose Your RPG Adventure</h1>
      <p class="text-xl max-w-2xl mx-auto">
        Select the type of RPG experience you want to create. This will shape the examples and style recommendations throughout the creation process.
      </p>
    </div>
    <!-- Game Mode Cards -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
      <!-- Chill / Cozy RPG Card -->
      <div class="game-mode-card group" data-mode="chill">
        <div class="game-mode-card-inner">
          <!-- Preview Image -->
          <div class="game-mode-image-container">
            <div class="game-mode-image bg-gradient-to-br from-green-100 to-blue-100 rounded-t-2xl h-48 flex items-center justify-center">
              <div class="text-center">
                <div class="text-6xl mb-2">🌿</div>
                <div class="text-sm font-medium">Calm & Exploratory</div>
              </div>
            </div>
          </div>
          <!-- Content -->
          <div class="game-mode-content">
            <h3 class="game-mode-title">Chill / Cozy RPG</h3>
            <p class="game-mode-description">
              Relaxed pace, exploration-focused gameplay with emphasis on storytelling, character development, and atmospheric world-building.
            </p>
            <ul class="game-mode-features">
              <li>✓ Peaceful exploration</li>
              <li>✓ Rich narrative depth</li>
              <li>✓ Character relationships</li>
              <li>✓ Low-stress gameplay</li>
            </ul>
          </div>
          <!-- Selection Indicator -->
          <div class="game-mode-selection-indicator">
            <div class="selection-dot"></div>
          </div>
        </div>
      </div>
      <!-- Action RPG Card -->
      <div class="game-mode-card group" data-mode="action">
        <div class="game-mode-card-inner">
          <!-- Preview Image -->
          <div class="game-mode-image-container">
            <div class="game-mode-image bg-gradient-to-br from-red-100 to-orange-100 rounded-t-2xl h-48 flex items-center justify-center">
              <div class="text-center">
                <div class="text-6xl mb-2">⚔️</div>
                <div class="text-sm font-medium">Fast & Exciting</div>
              </div>
            </div>
          </div> 
          <!-- Content -->
          <div class="game-mode-content">
            <h3 class="game-mode-title">Action RPG</h3>
            <p class="game-mode-description">
              Fast-paced, combat-driven experience with intense battles, quick decision-making, and adrenaline-pumping encounters.
            </p>
            <ul class="game-mode-features">
              <li>✓ Dynamic combat system</li>
              <li>✓ Quick reflexes required</li>
              <li>✓ Epic boss battles</li>
              <li>✓ High-intensity moments</li>
            </ul>
          </div> 
          <!-- Selection Indicator -->
          <div class="game-mode-selection-indicator">
            <div class="selection-dot"></div>
          </div>
        </div>
      </div>
    </div>
    <!-- Continue Button -->
    <div class="text-center mb-16">
      <button id="continue-btn" class="continue-button" disabled>
        Continue to Next Step
      </button>
      <p id="selection-hint" class="mt-4">
        Please select a game mode to continue
      </p>
    </div>
    <!-- Statistics Section -->
    <div class="bg-white rounded-2xl shadow-xl p-8 mb-8">
      <div class="text-center mb-8">
        <h2 class="text-3xl font-bold mb-2">Community Preferences</h2>
        <p class="">See what other creators are choosing</p>
      </div>
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <!-- Stats Table -->
        <div>
          <div class="bg-gray-50 rounded-xl p-6">
            <h3 class="text-xl font-bold mb-6">Selection Statistics</h3>
            <div class="space-y-4">
              <!-- Chill RPG Stats -->
              <div class="flex items-center justify-between p-4 bg-green-50 rounded-lg">
                <div class="flex items-center">
                  <div class="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center mr-4">
                    <span class="text-xl">🌿</span>
                  </div>
                  <div>
                    <h4 class="font-bold">Chill / Cozy RPG</h4>
                    <p class="text-sm">Calm & Exploratory</p>
                  </div>
                </div>
                <div class="text-right">
                  <div class="text-2xl font-bold" id="chill-count">0</div>
                  <div class="text-sm" id="chill-percentage">0%</div>
                </div>
              </div>
              <!-- Action RPG Stats -->
              <div class="flex items-center justify-between p-4 bg-red-50 rounded-lg">
                <div class="flex items-center">
                  <div class="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center mr-4">
                    <span class="text-xl">⚔️</span>
                  </div>
                  <div>
                    <h4 class="font-bold">Action RPG</h4>
                    <p class="text-sm">Fast & Exciting</p>
                  </div>
                </div>
                <div class="text-right">
                  <div class="text-2xl font-bold" id="action-count">0</div>
                  <div class="text-sm" id="action-percentage">0%</div>
                </div>
              </div>
              <!-- Total -->
              <div class="flex items-center justify-between p-4 bg-blue-50 rounded-lg mt-6">
                <div>
                  <h4 class="font-bold">Total Votes</h4>
                  <p class="text-sm">All community selections</p>
                </div>
                <div class="text-right">
                  <div class="text-2xl font-bold" id="total-count">0</div>
                  <div class="text-sm">users</div>
                </div>
              </div>
            </div>
            <div class="mt-8 text-center">
              <button id="reset-stats" class="text-sm hover:text-gray-900 underline">
                Reset Demo Data
              </button>
            </div>
          </div>
        </div>
        <!-- Chart -->
        <div>
          <div class="bg-gray-50 rounded-xl p-6 h-full">
            <h3 class="text-xl font-bold mb-6">Visual Distribution</h3>
            <div class="h-64">
              <canvas id="stats-chart"></canvas>
            </div>
            <div class="mt-6 text-center text-sm">
              <p>Real-time statistics from local database</p>
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- Info Note -->
    <div class="text-center text-sm">
      <p>Statistics are stored in a local SQLite database.</p>
    </div>
  </div>
</div>

<style>
  /* 简单的全局黑色字体规则 */
  body, .game-mode-card, .game-mode-content, .game-mode-title, .game-mode-description, 
  .game-mode-features, .game-mode-features li, .continue-button,
  .bg-gray-50, .bg-green-50, .bg-red-50, .bg-blue-50,
  h1, h2, h3, h4, p, span, div, button {
    color: #000000 !important;
  }

  .game-mode-card {
    border-radius: 1.5rem;
    box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);
    transition: all 0.3s ease;
    cursor: pointer;
    position: relative;
    overflow: hidden;
    background-color: white;
  }

  .game-mode-card:hover {
    transform: translateY(-8px);
    box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04);
  }

  .game-mode-card.selected {
    border: 3px solid #8b5cf6;
    transform: translateY(-4px);
  }

  .game-mode-card-inner {
    position: relative;
  }

  .game-mode-image-container {
    position: relative;
  }

  .game-mode-image {
    transition: all 0.3s ease;
  }

  .game-mode-card:hover .game-mode-image {
    transform: scale(1.05);
  }

  .game-mode-content {
    padding: 1.5rem;
  }

  .game-mode-title {
    font-size: 1.5rem;
    font-weight: bold;
    margin-bottom: 0.75rem;
  }

  .game-mode-description {
    margin-bottom: 1rem;
    line-height: 1.6;
  }

  .game-mode-features {
    list-style: none;
    padding: 0;
    margin: 0;
  }

  .game-mode-features li {
    margin-bottom: 0.5rem;
    font-weight: 500;
  }

  .game-mode-selection-indicator {
    position: absolute;
    top: 1rem;
    right: 1rem;
    width: 2rem;
    height: 2rem;
    border: 2px solid #000000;
    border-radius: 50%;
    display: flex;
    align-items: center;
    justify-content: center;
    transition: all 0.3s ease;
  }

  .game-mode-card.selected .game-mode-selection-indicator {
    border-color: #8b5cf6;
    background-color: #8b5cf6;
  }

  .selection-dot {
    width: 0.75rem;
    height: 0.75rem;
    border-radius: 50%;
    background-color: white;
    opacity: 0;
    transition: opacity 0.3s ease;
  }

  .game-mode-card.selected .selection-dot {
    opacity: 1;
  }

  .continue-button {
    background-color: #8b5cf6;
    color: white !important;
    padding: 0.75rem 2rem;
    border-radius: 0.75rem;
    font-weight: 600;
    font-size: 1.125rem;
    transition: all 0.3s ease;
    border: none;
    cursor: pointer;
    opacity: 0.5;
  }

  .continue-button:not(:disabled) {
    opacity: 1;
  }

  .continue-button:not(:disabled):hover {
    transform: translateY(-2px);
    box-shadow: 0 10px 15px -3px rgba(139, 92, 246, 0.3);
  }

  @keyframes pulse {
    0% { transform: scale(1); }
    50% { transform: scale(1.05); }
    100% { transform: scale(1); }
  }

  .game-mode-card.selected {
    animation: pulse 0.5s ease;
  }

  @keyframes countUp {
    from { transform: translateY(10px); opacity: 0; }
    to { transform: translateY(0); opacity: 1; }
  }

  .updated-stat {
    animation: countUp 0.5s ease;
  }
</style>

<script>
  const API_BASE_URL = 'http://localhost:8587';

  class StatisticsManager {
    constructor() {
      this.stats = {
        chill: 0,
        action: 0,
        total: 0,
        history: []
      };
      this.chart = null;
    }

    async loadStats() {
      try {
        const response = await fetch(`${API_BASE_URL}/api/stats`);
        if (response.ok) {
          this.stats = await response.json();
          this.updateDisplay();
        } else {
          console.error('Failed to load stats from server');
        }
      } catch (e) {
        console.error('Failed to connect to backend:', e);
        alert('Cannot connect to backend. Make sure Flask is running on port 5002');
      }
    }

    async recordSelection(mode) {
      const userId = this.getUserId();
      
      try {
        // 使用 GET 请求，参数在 URL 中
        const response = await fetch(`${API_BASE_URL}/api/stats/record?mode=${mode}&userId=${userId}`);

        if (response.ok) {
          this.stats = await response.json();
          this.updateDisplay();
          console.log(`Recorded selection: ${mode} by user ${userId}`);
        } else {
          console.error('Failed to record selection');
        }
      } catch (e) {
        console.error('Failed to connect to backend:', e);
        alert('Cannot connect to backend. Make sure Flask is running on port 5002');
      }
    }

    getUserId() {
      let userId = sessionStorage.getItem('rpgUserId');
      if (!userId) {
        userId = 'user_' + Math.random().toString(36).substr(2, 9);
        sessionStorage.setItem('rpgUserId', userId);
      }
      return userId;
    }

    getPercentage(mode) {
      if (this.stats.total === 0) return 0;
      return Math.round((this.stats[mode] / this.stats.total) * 100);
    }

    updateDisplay() {
      document.getElementById('chill-count').textContent = this.stats.chill;
      document.getElementById('action-count').textContent = this.stats.action;
      document.getElementById('total-count').textContent = this.stats.total;

      document.getElementById('chill-percentage').textContent = `${this.getPercentage('chill')}%`;
      document.getElementById('action-percentage').textContent = `${this.getPercentage('action')}%`;

      ['chill-count', 'action-count', 'total-count'].forEach(id => {
        const element = document.getElementById(id);
        element.classList.add('updated-stat');
        setTimeout(() => element.classList.remove('updated-stat'), 500);
      });

      this.updateChart();
    }

    updateChart() {
      if (!this.chart) return;

      this.chart.data.datasets[0].data = [this.stats.chill, this.stats.action];
      this.chart.update();
    }

    initChart() {
      const ctx = document.getElementById('stats-chart').getContext('2d');
      
      Chart.defaults.color = '#000000';
      Chart.defaults.font.family = "'Inter', 'Segoe UI', 'Roboto', sans-serif";
      Chart.defaults.font.size = 14;
      
      this.chart = new Chart(ctx, {
        type: 'doughnut',
        data: {
          labels: ['Chill / Cozy', 'Action'],
          datasets: [{
            data: [this.stats.chill, this.stats.action],
            backgroundColor: [
              'rgba(34, 197, 94, 0.8)',
              'rgba(239, 68, 68, 0.8)'
            ],
            borderColor: [
              'rgb(34, 197, 94)',
              'rgb(239, 68, 68)'
            ],
            borderWidth: 2,
            hoverOffset: 15
          }]
        },
        options: {
          responsive: true,
          maintainAspectRatio: false,
          color: '#000000',
          plugins: {
            legend: {
              position: 'bottom',
              labels: {
                padding: 20,
                usePointStyle: true,
                font: {
                  size: 14,
                  weight: '600'
                },
                color: '#000000'
              }
            },
            tooltip: {
              titleColor: '#000000',
              bodyColor: '#000000',
              backgroundColor: 'rgba(255, 255, 255, 0.95)',
              borderColor: '#d1d5db',
              borderWidth: 1,
              padding: 12,
              boxPadding: 6,
              callbacks: {
                label: (context) => {
                  const label = context.label || '';
                  const value = context.raw || 0;
                  const percentage = this.getPercentage(context.dataIndex === 0 ? 'chill' : 'action');
                  return `${label}: ${value} votes (${percentage}%)`;
                }
              }
            }
          }
        }
      });
    }

    async resetStats() {
      if (confirm('Are you sure you want to reset all statistics? This cannot be undone.')) {
        try {
          // 使用 GET 请求
          const response = await fetch(`${API_BASE_URL}/api/stats/reset`);

          if (response.ok) {
            this.stats = await response.json();
            this.updateDisplay();
            alert('Statistics have been reset.');
          } else {
            alert('Failed to reset statistics.');
          }
        } catch (e) {
          console.error('Failed to connect to backend:', e);
          alert('Cannot connect to backend. Make sure Flask is running on port 5002');
        }
      }
    }
  }

  document.addEventListener('DOMContentLoaded', function() {
    const gameModeCards = document.querySelectorAll('.game-mode-card');
    const continueBtn = document.getElementById('continue-btn');
    const selectionHint = document.getElementById('selection-hint');
    const resetStatsBtn = document.getElementById('reset-stats');
    let selectedMode = null;

    const statsManager = new StatisticsManager();

    statsManager.initChart();
    statsManager.loadStats();

    if (!sessionStorage.getItem('rpgCreatorPreferences')) {
      sessionStorage.setItem('rpgCreatorPreferences', JSON.stringify({}));
    }

    gameModeCards.forEach(card => {
      card.addEventListener('click', function() {
        const mode = this.dataset.mode;
        
        gameModeCards.forEach(c => c.classList.remove('selected'));
        
        this.classList.add('selected');
        
        selectedMode = mode;
        
        continueBtn.disabled = false;
        
        selectionHint.textContent = `Selected: ${getModeDisplayName(mode)} mode`;
        selectionHint.className = 'font-medium mt-4';
        
        const preferences = JSON.parse(sessionStorage.getItem('rpgCreatorPreferences'));
        preferences.gameMode = mode;
        sessionStorage.setItem('rpgCreatorPreferences', JSON.stringify(preferences));
        
        statsManager.recordSelection(mode);
        
        console.log('Game mode selected:', mode);
      });

      card.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' || e.key === ' ') {
          e.preventDefault();
          this.click();
        }
      });

      card.setAttribute('tabindex', '0');
    });

    continueBtn.addEventListener('click', function() {
      if (selectedMode) {
        const preferences = JSON.parse(sessionStorage.getItem('rpgCreatorPreferences'));
        preferences.gameMode = selectedMode;
        sessionStorage.setItem('rpgCreatorPreferences', JSON.stringify(preferences));
        
        // 跳转到 story 页面
        window.location.href = '/rpg/story';
      }
    });

    resetStatsBtn.addEventListener('click', function() {
      statsManager.resetStats();
    });

    function getModeDisplayName(mode) {
      const modeNames = {
        'chill': 'Chill / Cozy',
        'action': 'Action'
      };
      return modeNames[mode] || mode;
    }

    const preferences = JSON.parse(sessionStorage.getItem('rpgCreatorPreferences'));
    if (preferences.gameMode) {
      const previouslySelected = document.querySelector(`[data-mode="${preferences.gameMode}"]`);
      if (previouslySelected) {
        previouslySelected.click();
      }
    }
  });
</script>